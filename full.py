...
class Chatbot:
    ...
    def __init__(self) -> None:
        ...
        self.init_db()
        # self.system_prompt = {"role": "system", "content": "The output should be like a conversation from an experinced doctor. Explain in a short paragraph except when the user specifically says to."}
...
class DataExtractor:
    ...
    def extract_df_from_file(self, file_path: str) -> pd.DataFrame:
        ...
        # TODO - Comment and un Comment the following line to enable the file storage functionality
        # self.file_path = file_path
        # self.get_table_string()
        # self.get_markdown_table()
        # self.create_dataframe()

        self.extracted_df = pd.read_csv("algo_ops\\texts\\extracted_table.csv")
...
class ICliniq:
    ...
    def chat(self, user_input: str) -> str:
        ...
        if self.extracted_df is None:
            self.extracted_df = pd.DataFrame()
        # else:
        #     print("Data frame already extracted")
    ...
    def upload_file(self, category: str, file_path: str = "") -> None:
        ...
        # print("Gotten File Path: ", file_path)
...
def test():
    ...
    thing.upload_file("test", "sampleImages\\Drug Test Results.png")
    res = thing.chat("What is adviced in the report given ? Just mention the key information")
    # print(res)
    return res
...
if __name__ == "__main__":
    ...
    for _ in range(10):
        ...
        with open(f"testing_res\\result_{_+1}.txt", 'w') as f:
            f.write(str(final_op))
        print(f"Testing file {_+1} created")
