...
class Chatbot:
    ...
    def chat_with_model(self, user_id: int, user_input: str, query_df: pd.DataFrame = pd.DataFrame()) -> str:
        ...
        payload = {
            ...
            # "messages": conversation_history + [{"role": "user", "content": f"{user_input}, " + "```{}```".format(query_df.to_string() if not query_df.empty else "")}],
            # "messages": [{"role": "user", "content": f"{user_input}, " + (("```{}```".format(query_df.to_string())) if not query_df.empty else "")}],
